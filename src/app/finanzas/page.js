...
export default function FinanzasPage() {
  ...
  // NUEVO: Estado para guardar el nombre del archivo seleccionado
  ...
  // Listas de categorías dinámicas
  ...
  const iniciarEdicion = (tx) => {
    ...
    setNombreArchivo(''); // Reseteamos el archivo al editar
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 w-full max-w-7xl mx-auto">
        ...
        <div className={`p-6 rounded-lg shadow-lg border transition-all duration-300 h-fit ${idEditando ? 'bg-purple-900/20 border-purple-500' : 'bg-neutral-800 border-purple-900/50'}`}>
          ...
          <form className="flex flex-col gap-4">
            ...
            {/* NUEVO DISEÑO: Subida de Comprobante en Español */}
            <div className="mt-2">
            ...
            <button 
              type="button" 
              className={`font-bold py-3 px-4 rounded transition-colors mt-4 shadow-lg ${idEditando ? 'bg-amber-600 hover:bg-amber-700 shadow-amber-900/20 text-white' : 'bg-purple-600 hover:bg-purple-700 shadow-purple-900/20 text-white'}`}
            >
              {idEditando ? 'Actualizar Registro' : 'Guardar Registro Contable'}
            </button>
          </form>
        </div>
