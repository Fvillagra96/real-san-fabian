...
export default function TorneoExpressPage() {
  ...
  // ==========================================
  // FUNCIÓN NUEVA: CARGA MASIVA EXCEL
  // ==========================================
  const procesarCargaMasiva = async (e) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const mapaIDsFirebase = {}; // Aquí guardaremos: { "Nombre Equipo": "id_de_firebase" }

        // 1. Subir Equipos primero
        for (let eq of dataEquipos) {
          const nuevoEquipo = {
            nombre: eq["Nombre Equipo"],
            encargado: eq["Encargado / DT"],
            grupo: eq["Grupo"],
            pj: 0, fa: 0, pg: 0, pe: 0, pp: 0, gf: 0, gc: 0, dg: 0, pf: 0
          };
          const docRef = await addDoc(collection(db, 'torneo_equipos'), nuevoEquipo);
          mapaIDsFirebase[eq["Nombre Equipo"]] = docRef.id; // Vinculamos el nombre con el ID real
        }

        // 2. Subir Jugadores usando los IDs generados
        for (let jug of dataJugadores) {
          const nombreEq = jug["Equipo Perteneciente"];
          const equipoIdReal = mapaIDsFirebase[nombreEq]; // Buscamos a qué ID pertenece
        ...
        await cargarDatos(); // Refrescamos todo desde Firebase para mostrar los datos
      } catch (error) {
        console.error("Error leyendo Excel: ", error);
        alert("Hubo un error al leer el archivo. Revisa que tenga el formato de la plantilla.");
      } finally {
  ...
  const agregarJugador = async () => {
    ...
    const nuevoJugador = { nombre: nuevoNombreJugador, equipoId: equipoSeleccionadoId, goles: 0, amarillas: 0, rojas: 0 };
    try {
          ...
          <table className="w-full text-center text-sm text-gray-300 min-w-max">
            <thead className="bg-neutral-900/50 text-purple-400">
              <tr>
                ...
                <th className="p-2">PJ</th><th className="p-2">FA</th><th className="p-2">PG</th>
                <th className="p-2">PE</th><th className="p-2">PP</th><th className="p-2">GF</th>
                <th className="p-2">GC</th><th className="p-2">DG</th><th className="p-2 text-white font-bold">PF</th>
              ...
              {cargando ? (
                ...
                equiposFiltrados.map((eq, i) => (
                  <tr key={eq.id} className="border-b border-neutral-700/50 hover:bg-neutral-700/30 group">
                    ...
                    <td className="p-2 text-red-400">{eq.pp}</td><td className="p-2">{eq.gf}</td>
                    <td className="p-2">{eq.gc}</td><td className="p-2">{eq.dg}</td>
                    <td className="p-2 font-bold text-white bg-purple-900/20">{eq.pf}</td>
                    <td className="p-2"><button onClick={() => eliminarEquipo(eq.id)} className="text-red-500 hover:text-white hover:bg-red-600 p-1.5 rounded transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100">🗑️</button></td>
                  </tr>
                ))
              )}
            </tbody>
    ...
    <main className="flex min-h-screen flex-col p-4 md:p-8 relative">
      ...
      {tabActiva === 'partidos' && (
        <div className="w-full max-w-5xl mx-auto animate-fade-in flex flex-col gap-6 items-center text-center mt-10">
          ...
          ) : ( partidos.map(partido => <div key={partido.id}></div>) )}
        ...
        <div className="w-full max-w-5xl mx-auto animate-fade-in grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-neutral-800 p-6 rounded-lg border border-neutral-700 shadow-lg">
            ...
            <ul className="flex flex-col gap-2">
              {cargando ? <li className="text-purple-400 animate-pulse p-2">Cargando goleadores...</li> : jugadores.filter(j => j.goles > 0).length === 0 ? (
                ...
                jugadores.filter(j => j.goles > 0).sort((a,b) => b.goles - a.goles).map((jug, index) => (
                  <li key={jug.id} className="flex justify-between items-center p-2 bg-neutral-900/50 rounded border border-neutral-800 hover:bg-neutral-800 transition-colors">
                    <div className="flex gap-3"><span className="text-gray-500 font-bold w-4">{index + 1}.</span><span className="text-white font-medium">{jug.nombre}</span></div><span className="text-purple-400 font-bold">{jug.goles} Goles</span>
                  </li>
                ))
              )}
            </ul>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                <select value={formEquipo.grupo} onChange={e => setFormEquipo({...formEquipo, grupo: e.target.value})} className="w-full p-2.5 rounded bg-neutral-900 text-white border border-neutral-700 focus:border-purple-500 outline-none transition-colors">
                  <option value="">Seleccionar Grupo...</option><option value="Grupo A Varones">Grupo A Varones</option><option value="Grupo B Varones">Grupo B Varones</option><option value="Damas">Damas</option>
                </select>
              </div>
            </div>
      ...
      {modalJugador && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 animate-fade-in backdrop-blur-sm">
          <div className="bg-neutral-800 p-6 rounded-lg w-full max-w-lg border border-purple-900/50 shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <select value={equipoSeleccionadoId} onChange={e => setEquipoSeleccionadoId(e.target.value)} className="w-full p-2.5 rounded bg-neutral-900 text-white border border-purple-900 focus:border-purple-500 outline-none mb-4 transition-colors">
              <option value="">-- Elige un equipo --</option>{equipos.map(eq => <option key={eq.id} value={eq.id}>{eq.nombre} ({eq.grupo})</option>)}
            </select>
              ...
              <div className="flex-1 overflow-y-auto flex flex-col gap-4 pr-1">
                <div className="bg-neutral-900/50 p-3 rounded border border-neutral-700 shadow-inner">
                  ...
                  <div className="flex gap-2">
                    <input type="text" value={nuevoNombreJugador} onChange={e => setNuevoNombreJugador(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && agregarJugador()} placeholder="Nombre del jugador" className="flex-1 p-2 rounded bg-neutral-900 text-white border border-neutral-700 focus:border-purple-500 outline-none transition-colors"/>
                    ...
                    {jugadores.filter(j => j.equipoId === equipoSeleccionadoId).map((jug, idx) => (
                      <li key={jug.id} className="flex justify-between items-center p-2 bg-neutral-900/80 rounded border border-neutral-700 hover:border-neutral-600 transition-colors">
                        ...
                        <div className="flex gap-2 shrink-0">
                          {jugadorEditando === jug.id ? <button onClick={() => guardarEdicionJugador(jug.id)} className="text-green-400 hover:text-green-300 bg-neutral-800 px-2 py-1 rounded text-xs font-bold border border-green-900 transition-colors">Guardar</button> : <button onClick={() => iniciarEdicionJugador(jug)} className="text-purple-400 hover:text-purple-300 bg-neutral-800 px-2 py-1 rounded text-xs border border-purple-900 transition-colors">✏️</button>}
