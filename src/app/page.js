...
export default function TorneoExpressPage() {
  ...
  const guardarCambiosParciales = async () => {
    ...
    try {
      ...
      await cargarDatos();
    } catch (error) { console.error(error); alert("Error guardando progreso."); }
  };
  ...
  const finalizarPartido = async () => {
    ...
    try {
      ...
      // 3. Finalizar Partido y guardar caja negra de estadísticas
      await updateDoc(doc(db, 'torneo_partidos', partidoActivo.id), {
        estado: 'Finalizado',
        golesLocal: golesLoc,
        golesVisita: golesVis,
    ...
    } catch (error) { console.error(error); alert("Error guardando datos."); }
  };
  ...
  const revertirPartido = async () => {
    ...
    try {
        ...
        alert("🔙 Partido revertido con éxito. Ahora está 'Pendiente' para que corrijas los errores.");
        setPartidoActivo(null);
        await cargarDatos();
  ...
  return (
    <main className="flex min-h-screen flex-col p-2 md:p-8 relative max-w-7xl mx-auto mt-4 md:mt-0">
      ...
      {/* ========================================= */}
      {/* MODAL: MESA DE CONTROL CON BOTÓN DE GUARDADO EN VIVO Y REVERTIR */}
      {/* ========================================= */}
      {partidoActivo && (
...
```Entiendo el dolor de cabeza con los duplicados. Como el sistema no sabe qué hacer con los datos anteriores, simplemente amontona lo nuevo sobre lo viejo. 

Para evitar esto, la solución más limpia es hacer un "barrido general" en la categoría (ya sean equipos o partidos) justo un segundo antes de que proceses la subida masiva. Es decir, vaciar la mesa antes de servir los platos nuevos.

En tu base de datos, puedes crear una función que agrupe todos los registros existentes y los elimine de un solo golpe (usando un `batch`, que es como meter todo en una misma bolsa para borrarlo más rápido). 
